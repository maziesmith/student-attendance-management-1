...
class mainClass
{
	...
	public function userLogin($email,$password,$ip)
	{
		try{
			...
			$stmt = $db->prepare("SELECT * FROM users where email=:email and password=:passwordd ");
			$stmt->bindParam("email", $email,PDO::PARAM_STR) ;
			$stmt->bindParam("passwordd", $hash_password,PDO::PARAM_STR) ;
			$res1=$stmt->execute();
			if($res1)
			{
				...
				if($data['role']!='admin')
				{
					$stmt2 = $db->prepare("SELECT * FROM ip_addr where user_id=:id ");
					$stmt2->bindParam("id", $data['id'],PDO::PARAM_STR) ;
					$res2=$stmt2->execute();
					$newdata=$stmt2->fetch(PDO::FETCH_ASSOC);
					//echo "Correct ".$newdata['ip'];
					if($newdata['ip']==$ip)
					{
...
JOIN program ON students.program_id = program.id WHERE month = $month AND year = $year GROUP BY student_id LIKE :searchString");
			...
			$d1=$data[0]->student_id;
			$d2=$data[1]->student_id;
			$d3=$data[2]->student_id;
			$d4=$data[3]->student_id;
			$d5=$data[4]->student_id;
			$d6=$data[5]->student_id;
			$d7=$data[6]->student_id;
			$d8=$data[7]->student_id;
			$d9=$data[8]->student_id;
			$d10=$data[9]->student_id;
			//echo $pl;
			$datelike1='%/'.$month.'/'.$year;
			$datelike2='%/0'.$month.'/'.$year;
			$stmt = $db->prepare("SELECT * from dateattendance where student_id IN ('$d1','$d2','$d3','$d4','$d5','$d6','$d7','$d8','$d9','$d10') AND (dates like '$datelike1' OR dates like '$datelike2' ) " );
			$stmt->bindParam("searchString", $searchString,PDO::PARAM_STR);
			$stmt->execute();
...
public function saveRecords($month,$year,$student_id,$rollno,$program_name,$presents,$absents,$sl,$ml,$cl,$maternity,$duty,$work_off,$form_submit,$currdate)
	{
		...
		if($month >= 7) {
			...
			$query = "SELECT COUNT(*) as total, SUM(sanctioned) sanctioned, SUM(contingency) cl, SUM(medical) ml, SUM(maternity) maternity, SUM(duty) duty , SUM(work_off) work_off  FROM attendance WHERE student_id = :student_id AND (month > :month AND year = :year -1) OR year = :year GROUP BY student_id";
...
//-------------------------------------------------Shubh-------------------------------------------------------------------------------------------------------------------------//
		...
		// $date_str = "31/12/".$previous_year;
		// $stmt_med = $db->prepare("SELECT COUNT(*) as count FROM dateattendance WHERE student_id =".$student_id." AND str_to_date(dates,'%d/%m/%Y') < str_to_date('".$date_str."','%d/%m/%Y') AND type='ml'");
		// $stmt_med->execute();
		// $allowed_count=$stmt2->fetchAll(PDO::FETCH_OBJ);
		// $previous_medical_count = $allowed_count[0]->count;
		...
		// echo $previous_count[0]->cl.'<br>';
			...
			if($month>=6 && $month<=12)
			{
				...
				//$currentyear = date('y');
				$upper_date_str="31/12/".$year;
				$lower_date_str="01/07/".$year;
				$stmt_med = $db->prepare("SELECT COUNT(*) as count FROM dateattendance WHERE student_id =".$student_id." AND str_to_date(dates,'%d/%m/%Y') <= str_to_date('".$upper_date_str."','%d/%m/%Y') AND str_to_date(dates,'%d/%m/%Y') >= str_to_date('".$lower_date_str."','%d/%m/%Y') AND type='ml'");
				$stmt_med->execute();
				$prev_med=$stmt_med->fetchAll(PDO::FETCH_OBJ);
				$prev_med_count=$prev_med[0]->count;
				if($allowed_count->medical - $prev_med_count < $ml )
					return -3;
			...
			else if($month>=1 && $month <=5)
			{
				...
				$prevyear=$year-1;
				$date_str="31/12/".$prevyear;
				$stmt_med = $db->prepare("SELECT COUNT(*) as count FROM dateattendance WHERE student_id =".$student_id." AND str_to_date(dates,'%d/%m/%Y') <= str_to_date('".$date_str."','%d/%m/%Y') AND type='ml'");
				$stmt_med->execute();
				$prevyear_med=$stmt_med->fetchAll(PDO::FETCH_OBJ);
				$prevyear_med_count=$prevyear_med[0]->count;
				if($prevyear_med_count<8)
				{
					$allowed_medical_count=8;
				}
				else
				{
					$allowed_medical_count=7;
				}
	
				$date_str="01/01/".$year;
				$stmt_med2 = $db->prepare("SELECT COUNT(*) as count FROM dateattendance WHERE student_id =".$student_id." AND str_to_date(dates,'%d/%m/%Y') >= str_to_date('".$date_str."','%d/%m/%Y') AND type='ml'");
				$stmt_med2->execute();
				$prev_med=$stmt_med2->fetchAll(PDO::FETCH_OBJ);
				$prev_med_count=$prev_med[0]->count;
				if($allowed_medical_count - $prev_med_count < $ml )
					return -3;
			}
		...
		//-------------------------------------Shubh------------------------------------------------------------//
		// if($allowed_count->medical >= 8)
		// $allowed_count->medical = 7;
		// else
		// $allowed_count->medical = 8;
		//------------------------------------------------------------------------------------------------------//
		
		//------------------------Tarun ( removed this medical count because new conditions have been put up)-------//
